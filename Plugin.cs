...
namespace DefusingExpert
{
    public class Plugin : BasePlugin, IPluginConfig<Configs>
    {
        public override string ModuleName => "The Defusing Expert Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerDefusingCount> _playerBombCount { get; set; } = new();
        ...
        [GameEventHandler]
        public HookResult OnPlayerDeath(EventBombDefused @event, GameEventInfo info)
        {
            var client = @event.Userid;
        ...
        public void CountBomb(CCSPlayerController client)
        {
            ...
            if (_playerBombCount[client!].BombCount >= Config.MaxDefusingCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Defusing Expert' (Defusing the bomb for {Config.MaxDefusingCount} times)");
